-- =====================================================
-- Function: get_user_total_balances
-- Description: Calculate total balance details for a user across all groups
-- Returns: user's total balance, who they owe, and who owes them (aggregated across all groups)
-- Note: Aggregates balances from all groups the user is part of
-- Author: Split App Team
-- Created: 2025-01-XX
-- =====================================================

-- Drop function if exists (for clean re-runs)
DROP FUNCTION IF EXISTS public.get_user_total_balances(UUID);

-- =====================================================
-- Main Function: get_user_total_balances
-- Explicitly create in public schema
-- =====================================================
CREATE OR REPLACE FUNCTION public.get_user_total_balances(
  p_user_id UUID
)
RETURNS TABLE (
  user_id UUID,
...
BEGIN
  -- Step 1: Calculate total paid by user across all groups (from expenses)
  SELECT COALESCE(SUM(ep.amount), 0)::DECIMAL(10,2)
  INTO v_total_paid
  FROM expense_payers ep
  JOIN expenses e ON e.id = ep.expense_id
  WHERE ep.paid_by = p_user_id;

  -- Step 2: Calculate total owed by user across all groups (from expense splits)
  ...
  -- Step 3: Calculate net balance including payments across all groups
  ...
  v_net_balance := v_net_balance - COALESCE((
    SELECT SUM(p.amount)
    FROM payments p
    WHERE p.to_user_id = p_user_id
  ), 0);

  -- Calculate final net balance
  v_net_balance := v_total_paid + v_net_balance - v_total_owed;

  -- Step 4: Calculate who the user owes (aggregated across all groups)
  ...
  -- Subtract payments made to each creditor (across all groups)
  net_debts AS (
    SELECT 
      ed.creditor_id,
  ...
  -- Step 5: Calculate who owes the user (aggregated across all groups)
  ...
  WITH expense_credits AS (
    ...
    WHERE ep.paid_by = p_user_id  -- User paid for this expense
      AND es.user_id != p_user_id  -- Don't count user's own split
    GROUP BY es.user_id
  ),
  -- Subtract payments received from each debtor (across all groups)
  net_credits AS (
    SELECT 
      ec.debtor_id,
...
GRANT EXECUTE ON FUNCTION public.get_user_total_balances(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_total_balances(UUID) TO service_role;

-- Add helpful comment
COMMENT ON FUNCTION public.get_user_total_balances IS 
'Calculates total balance details for a user across all groups. Returns user_id, total_paid, total_owed, net_balance, owes_to (JSONB array of users and amounts the user owes aggregated across all groups), and owed_by (JSONB array of users and amounts who owe the user aggregated across all groups). This function aggregates balances from all groups the user is part of, providing a complete financial overview.';
