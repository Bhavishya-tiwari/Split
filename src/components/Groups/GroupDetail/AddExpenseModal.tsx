...
interface SplitInput {
  userId: string;
  splitType: SplitType;
  value?: string; // For percentage, exact amount, or shares
}
...
export default function AddExpenseModal({
  ...
  const toggleSplitMember = (userId: string) => {
    ...
    if (newSelected.has(userId)) {
      ...
      // Clear exact amount if unselecting
  ...
  const calculateSplits = (): SplitInput[] => {
    ...
    // Check if only the payer is selected (can't split expense only to the payer)
    ...
    if (splitType === SplitType.EQUAL) {
      ...
      // Exact amounts - use manually entered amounts
      const totalExact = getTotalExactAmounts();
      
      // Validate that exact amounts are provided for all selected members
      ...
      // Validate that exact amounts sum to total paid
  ...
  const onSubmit = async (data: ExpenseFormData) => {
    ...
    try {
      ...
      // TODO: Create API endpoint to handle expense creation
      console.log('Creating expense:', expenseData);
      
      // For now, we'll show a success message
      // await axios.post('/api/expenses', expenseData);
      
      alert('Expense creation functionality will be implemented with the API endpoint');
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto">
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2 mb-4">
                  {members.map((member) => (
                    ...
                    >
                      ...
                      {/* Show calculated amount for EQUAL split */}
                      ...
                      {/* Exact Amount Input - only show if EXACT split type and member is selected */}
                ...
                {/* Warning if only payer is selected */}
                {selectedSplitMembers.size === 1 && Array.from(selectedSplitMembers)[0] === watch('paidBy') && (
                  <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
                    <span className="text-amber-600 text-sm">⚠️</span>
                    <p className="text-sm text-amber-700">
                      Cannot split an expense only to {
                        (() => {
                          const paidBy = watch('paidBy');
                          const payerMember = members.find(m => m.profiles.id === paidBy);
                          return payerMember?.profiles.display_name || payerMember?.profiles.email || 'the payer';
                        })()
                      }. Please include at least one other member.
                    </p>
                  </div>
                )}
                
                {/* Show summary for equal split */}
                ...
                {/* Show validation summary for exact amounts */}
