 ...
 * Returns balance details for the authenticated user in a group
 * Shows who they owe and who owes them
 */
export async function GET(
  ...
  try {
    ...
    // Call SQL function to get user's balance
    ...
    if (balanceError) {
      ...
      if (errorMessage.includes('function') && errorMessage.includes('does not exist')) {
        return NextResponse.json(
          { 
            error: 'Database function not found. Please run the get_user_balance migration.',
    ...
    console.error('Error in settlements route:', error);
