...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: mappingError } = await serviceSupabase.from('user_group_mapping').insert({
      user_id: user.id,
      group_id: group.id,
      role: 'admin',
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (membership.role !== 'admin') {
      return NextResponse.json({ error: 'Only admins can update groups' }, { status: 403 });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error: deleteError } = await serviceSupabase.from('groups').delete().eq('id', id);
